...
def Build(hostPlatform, hostArch, buildSys, compiler, arch, configuration, tblgenMode, tblgenPath):
	...
	if (buildSys == "ninja"):
		...
		if (configuration == "clangformat"):
			...
			options = f'-DCMAKE_BUILD_TYPE="{configuration}" -DSC_ARCH_NAME="{arch}" {tblgenOptions} -D' % (configuration, arch, tblgenOptions)
...
if __name__ == "__main__":
	...
	if (hostArch == "AMD64") or (hostArch == "x86_64"):
		...
		hostArch = "x86"
	elif (hostArch.lower() == "arm64" or hostArch.lower() == "aarch64"):
		hostArch = "arm64"
	else:
